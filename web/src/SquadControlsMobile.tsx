...
import { GREEN, INK, MUTED2, PANEL_MOBILE } from './theme'
...
const SLOT_STYLE = {
  padding: '8px 9px', background: '#f1efe8', border: '1px solid rgba(22,21,15,.16)',
  minWidth: 0, // lets the Select's text-overflow ellipsis actually kick in inside a grid track
}

// reuses the same collapsible-sheet pattern (chevron + title + count) as the ladder sections below
// it, so it reads as "tap to expand" the same way those already do, instead of a bespoke "Edit" link
export function SquadControlsMobile({
  ...
  return (
    <div style={{ padding: '8px 22px 22px', background: PANEL_MOBILE, borderBottom: '1px solid rgba(22,21,15,.14)' }}>
      <MobileSheet title="Your squad" count={`£${budget.toFixed(1)}M budget`}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 10 }}>
          <span style={{ font: '400 12.5px/1 Archivo,sans-serif', color: MUTED2 }}>Budget</span>
          <span style={{ font: '500 15px/1 Archivo,sans-serif', color: INK }}>£{budget.toFixed(1)}M</span>
        </div>
        <input
          ...
          style={{ width: '100%', accentColor: GREEN, height: 20, marginBottom: 4 }}
        />
        ...
        <p style={{ margin: '0 0 10px', font: '400 12.5px/1 Archivo,sans-serif', color: MUTED2 }}>Your squad</p>
        ...
        <button
          ...
          style={{
            width: '100%', padding: '12px 16px', border: 'none', minHeight: 44,
